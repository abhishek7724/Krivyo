...
function accessParam(){const h=location.hash.startsWith('#')?location.hash.slice(1):location.hash;return new URLSearchParams(h).get('access')}
...
async function loadLiveCapture(captureId,accessToken){
  ...
  const response=await fetch(endpoint,{
    ...
    body:JSON.stringify({action:'load',capture_id:captureId,access_token:accessToken})
  });
  let payload=null;try{payload=await response.json()}catch{}
  if(!response.ok)throw new Error(payload?.details||payload?.error||`Workspace API returned HTTP ${response.status}`);
...
function showLoadError(message){
  $('#main').innerHTML=`<div style="padding:56px;max-width:760px">
    ...
    <p style="margin-top:22px;color:#667085">Open the secure Workspace link created for this capture.</p>
...
async function init(){
  try{
    ...
    if(captureId){
      ...
      if(!accessToken)throw new Error('Secure access token is missing from this Workspace link.');
      const live=await loadLiveCapture(captureId,accessToken);
